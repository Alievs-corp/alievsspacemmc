    ...
    <div className='flex justify-center mt-[80px] md:mt-[120px]'>
      <div className="max-w-6xl w-full px-4 py-8">
        <div className="text-center mb-10">
          <h1 className="font-display text-[26px] md:text-[38px] font-bold text-white mb-4">{t('public.premiumProcess')}</h1>
          <p className="font-inter text-text-muted text-[13px] md:text-[18px] mx-auto max-w-3xl">{t('public.processDesc')}</p>
        </div>

        <div className="relative mt-[40px] md:mt-[60px] lg:mt-[80px]">
          <div className="hidden md:block absolute top-1/2 left-0 right-0 h-px bg-border -translate-y-1/2"></div>
            ...
            {phases.slice(0, 3).map((phase) => (
              <div key={phase.quarter} className="flex items-start md:w-[30%]">
                <div className="flex-shrink-0 mr-4 md:mr-6">
                  ...
                  >
                    <span className="text-primary font-mono font-bold text-[24px] border-b border-l border-border-strong px-4 py-2">
                      {phase.quarter}
                    </span>
                  </div>
                </div>
                ...
                <div className="pt-1">
                  <h3 className="text-white font-display font-semibold text-xl mb-2">{phase.title}</h3>
                  <p className="text-text-muted text-base">{phase.desc}</p>
                </div>
              </div>
            ))}
          </div>
              ...
              <div key={phase.quarter} className="flex items-start md:w-[30%]">
                <div className="flex-shrink-0 mr-4 md:mr-6">
                  <div 
                    ...
                    style={{ borderRadius: '6.45px' }}
                  >
                    <span className="text-primary font-mono font-bold text-[24px] border-t border-r border-border-strong px-4 py-2">
                      {phase.quarter}
                    </span>
                  </div>
                </div>
